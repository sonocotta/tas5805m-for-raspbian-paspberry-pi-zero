...
#define DSP_BOOT_DELAY_US      5000
#define DSP_POST_BOOT_DELAY_US 10000
...
static void tas5805m_refresh(struct snd_soc_component *component)
{
    ...
    dev_dbg(component->dev, "Refreshing the component\n");
	dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_PAGE, 0x00);
    regmap_write(rm, REG_PAGE, 0x00);
	dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_BOOK, 0x00);
    regmap_write(rm, REG_BOOK, 0x00);
	dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_PAGE, 0x00);
    regmap_write(rm, REG_PAGE, 0x00);

	dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_DEVICE_CTRL_2, (tas5805m->is_muted ? DCTRL2_MUTE : 0) | DCTRL2_MODE_PLAY);
    regmap_write(rm, REG_DEVICE_CTRL_2,
        (tas5805m->is_muted ? DCTRL2_MUTE : 0) | DCTRL2_MODE_PLAY);
    dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_FAULT, ANALOG_FAULT_CLEAR);
...
static void send_cfg(struct regmap *rm, const uint8_t *s, unsigned int len)
{
    unsigned int i;
    int ret;

    dev_dbg(rm->dev, "Sending configuration to the device\n");

    for (i = 0; i + 1 < len; i += 2) {
		dev_dbg(rm->dev, "\tregmap_write: %#02x %#02x", s[i], s[i + 1]);
        ret = regmap_write(rm, s[i], s[i + 1]);
        if (ret != 0) {
            dev_err(rm->dev, "Error writing register %#02x: %d\n", s[i], ret);
            return; // Stop sending configuration if an error occurs
        }
    }
}
 ...
 * sound.
 */
static int tas5805m_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
    ...
    dev_dbg(component->dev, "Trigger command: %d\n", cmd);
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		 ...
		 */
        usleep_range(DSP_BOOT_DELAY_US, DSP_POST_BOOT_DELAY_US);
    ...
    case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		dev_dbg(component->dev, "\tregmap_write: %#02x %#02x", REG_DEVICE_CTRL_2, DCTRL2_MODE_PLAY | DCTRL2_MUTE);
        regmap_write(rm, REG_DEVICE_CTRL_2, DCTRL2_MODE_PLAY | DCTRL2_MUTE);
        break;

    default:
        dev_err(component->dev, "Invalid trigger command: %d\n", cmd);
        return -EINVAL;
    }
...
static int tas5805m_mute(struct snd_soc_dai *dai, int mute, int direction)
{
    ...
    dev_dbg(component->dev, "Setting mute: %d (is_powered=%d)\n", mute, tas5805m->is_powered);
...
static int tas5805m_i2c_probe(struct i2c_client *i2c)
{
    ...
    dev_dbg(dev, "Probing the I2C device\n");
    ...
    if (!tas5805m) {
        dev_err(dev, "Failed to allocate memory for tas5805m_priv\n");
        return -ENOMEM;
    }
    ...
    dev_dbg(dev, "Requesting firmware\n");
    ...
    if (ret) {
        dev_info(dev, "ti,dsp-config-name property not found, using default firmware\n");
        config_name = "default";
    }

    ret = snprintf(filename, sizeof(filename), "tas5805m_dsp_%s.bin", config_name);
    if (ret < 0 || ret >= sizeof(filename)) {
        dev_err(dev, "Error formatting firmware filename: %d\n", ret);
        return -EINVAL;
    }
    ...
    if (ret) {
        ...
        fw = NULL; // Indicate firmware is not loaded
    } else if ((fw->size < 2) || (fw->size & 1)) {
    ...
    if (fw) {
        ...
        if (!tas5805m->dsp_cfg_data) {
            dev_err(dev, "Failed to allocate memory for firmware data\n");
            release_firmware(fw);
            return -ENOMEM;
        }
    ...
    dev_dbg(dev, "Powering on the device\n");
    ...
    ret = gpiod_set_value(tas5805m->gpio_pdn_n, 1);
    if (ret) {
        dev_err(dev, "Failed to set PDN gpio: %d\n", ret);
...
static void tas5805m_i2c_remove(struct i2c_client *i2c)
{
    ...
    dev_dbg(dev, "Removing the I2C device\n");

    // Unregister the component
    ret = snd_soc_unregister_component(dev);
    if (ret != 0) {
        dev_err(dev, "Error unregistering codec: %d\n", ret);
    }

    // Deassert PDN#
    ret = gpiod_set_value(tas5805m->gpio_pdn_n, 0);
    if (ret) {
        dev_err(dev, "Failed to set PDN gpio: %d\n", ret);
    }
    ...
    ret = regulator_disable(tas5805m->pvdd);
    if (ret) {
        dev_err(dev, "Failed to disable pvdd: %d\n", ret);
    }

    // Free the firmware data if it was loaded
    if (tas5805m->dsp_cfg_data) {
        devm_kfree(dev, tas5805m->dsp_cfg_data);
    }
}
